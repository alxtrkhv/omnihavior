...
namespace Omnihavior.Core
{
  ...
  {
    public static SequenceNode<TInputData> AsSequence<TInputData>(
      this IReadOnlyList<IBehaviorNode<TInputData>> children,
    ...
    {
      return Builder.Default<TInputData>().Sequence(new[] { firstNode, secondNode, }, rules);
    }

    public static SelectorNode<TInputData> AsSelector<TInputData>(
      this IReadOnlyList<IBehaviorNode<TInputData>> children,
    ...
    {
      return Builder.Default<TInputData>().Selector(new[] { firstNode, secondNode, }, rules);
    }

    public static ParallelNode<TInputData> InParallel<TInputData>(
      this IReadOnlyList<IBehaviorNode<TInputData>> children,
    ...
    {
      return Builder.Default<TInputData>().Fake(pattern ?? Array.Empty<NodeStatus>());
    }

    public static FakeNode<TInputData> SubstituteWithFake<TInputData>(this IBehaviorNode<TInputData> _,
      NodeStatus? status = null)
    {
      return Builder.Default<TInputData>()
        .Fake(status is not null ? new[] { status.Value, } : Array.Empty<NodeStatus>());
    }
  }
}
